...
const SettingsTab = ({ inventory, activeSubTab, setActiveSubTab, setItemForm, setShowItemModal }) => {
  return (
          ...
          <tbody className="divide-y text-slate-600">
            {inventory.filter(i => i.category === activeSubTab).map(item => (
              <tr key={item.id} className="hover:bg-slate-50 transition">
                ...
                <td className="p-4 text-xs font-bold text-slate-400 whitespace-nowrap">{item.unit}</td>
                <td className="p-4 text-right text-blue-600 font-bold font-mono whitespace-nowrap">$ {item.unitPrice.toLocaleString()}</td>
                <td className="p-4 text-center whitespace-nowrap"><button onClick={() => { setItemForm({ ...item, unitPrice: String(item.unitPrice) }); setShowItemModal(true); }} className="text-slate-300 hover:text-blue-600 p-2"><Pencil size={16} /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
